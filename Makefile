...
LIST_SRCS = list.c
LIST_HDRS = $(LIST_SRCS:.c=.h)

VECTOR_TEST_SRCS = /test/vectortest.c $(VECTOR_SRCS)
...
LIST_TEST_SRCS = list_test.c $(LIST_SRCS)
LIST_TEST_OBJS = $(LIST_TEST_SRCS:.c=.o)
...
SRCS = $(LIST_SRCS) $(VECTOR_SRCS) $(HASHSET_SRCS) $(ST_SRCS) vectortest.c hashsettest.c list_test.c
HDRS = $(LIST_HDRS) $(VECTOR_HDRS) $(HASHSET_HDRS) $(ST_HDRS)

EXECUTABLES = list-test vector-test hashset-test thesaurus-lookup
PURIFY_EXECUTABLES = list-test vector-test-pure hashset-test-pure thesaurus-lookup-pure
...
list-test : Makefile.dependencies $(LIST_TEST_OBJS)
	$(CC) -o $@ $(LIST_TEST_OBJS) $(LDFLAGS)
